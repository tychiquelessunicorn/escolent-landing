...
export function BranchingPathSection() {
  ...
  // Continuous scroll-linked progression across Side-by-Side, Scaffold Ladder, and Moving Ahead views
  ...
  const branchSpread = useTransform(scrollYProgress, [0.05, 0.25], [0.94, 1]);
  const forkLineProgress = useTransform(scrollYProgress, [0.02, 0.20], [0, 1]);
  const originOpacity = useTransform(scrollYProgress, [0.01, 0.15], [0, 1]);
    ...
    >
      <div className="sticky top-0 min-h-screen py-16 sm:py-20 px-4 sm:px-6 md:px-8 flex flex-col items-center justify-center overflow-hidden">
        <div className="max-w-7xl mx-auto w-full flex flex-col items-center">
          ...
          >
            <div className="p-3 rounded-[16px] bg-[var(--bg-surface)] border border-[var(--border-medium)] relative shadow-md">
              <div className="text-[10px] font-semibold text-[var(--text-muted)] mb-0.5">
                Common Starting Point · Multi-Step Linear Equation
              </div>
              <p className="text-xs sm:text-sm font-display font-medium text-[var(--text-primary)]">
                Solve for x: <span className="text-[var(--brand-highlight)] font-semibold font-mono">5x + 3 = 2x + 18</span>
              </p>
            </div>
          </motion.div>
          ...
          <motion.div
            ...
            className="w-full grid grid-cols-1 lg:grid-cols-2 gap-6 items-start mb-6"
          >
            ...
            {(activeTab === "both" || activeTab === "scaffold") && (
              ...
              >
                ...
                <LiveIframe
                  ...
                  height={activeTab === "both" ? "h-[380px]" : "h-[440px]"}
                />

                {/* Shell Footer Notes */}
                <div className="p-3 bg-[var(--bg-surface-elevated)] border-t border-[var(--border-subtle)] flex items-center justify-between text-xs text-[var(--text-secondary)]">
                  <span className="text-[11px]">Step-down scaffolding activated upon wrong attempt</span>
                  <a
                    href={scaffoldUrl}
                    target="_blank"
                    rel="noopener noreferrer"
          ...
          <div className="w-full max-w-5xl p-4 sm:p-5 rounded-[22px] bg-[var(--bg-surface)] border border-[var(--border-subtle)] shadow-lg">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 mt-3">
              {masteryStages.map((stage, idx) => (
                <motion.div
                  ...
                  whileTap={{ scale: 0.98 }}
                  className={`p-2.5 rounded-[12px] flex flex-col justify-between cursor-pointer transition-all duration-200 ${stage.color} ${
