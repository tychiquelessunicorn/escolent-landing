...
export function ThreeRolesSection() {
  ...
  // Extended scroll track giving each role substantial dwell time
  ...
  useMotionValueEvent(scrollYProgress, "change", (progress) => {
    // 0.00 -> 0.38: Student Practice
    if (progress < 0.38) {
      if (activeTab !== "student") setActiveTab("student");
    } 
    // 0.38 -> 0.72: Teacher Escalations
    else if (progress < 0.72) {
      if (activeTab !== "teacher") setActiveTab("teacher");
    } 
    // 0.72 -> 1.00: Admin Briefing & Governance
    else {
    ...
    >
      <div className="sticky top-0 min-h-screen py-8 sm:py-12 px-4 sm:px-6 md:px-8 flex flex-col items-center justify-center overflow-hidden">
        <div className="max-w-7xl mx-auto w-full flex flex-col items-center justify-between">
          ...
          <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-3 gap-2.5 mb-3.5">
            {steps.map((step) => {
              ...
              return (
                <motion.button
                  ...
                  whileHover={{ y: -1 }}
                  whileTap={{ scale: 0.98 }}
                  className={`text-left p-2.5 sm:p-3 rounded-[16px] border transition-all duration-200 flex flex-col justify-between relative overflow-hidden ${
                    isActive
                ...
                >
                  <div className="flex items-center justify-between mb-1.5">
                    <span
                      className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-[6px] ${
                        isActive
                    ...
                    {step.id === "student" && (
                      <GraduationCap
                        className={`w-3.5 h-3.5 ${isActive ? "text-[var(--brand-text)]" : "text-[var(--text-muted)]"}`}
                      />
                    )}
                    {step.id === "teacher" && (
                      <UserCheck
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-[var(--text-muted)] mt-0.5">{step.headline}</p>
                  </div>
          ...
          <motion.div
            ...
            className="w-full max-w-5xl rounded-[20px] bg-[var(--bg-surface)] border border-[var(--border-medium)] shadow-xl overflow-hidden mb-3"
          >
            {/* Header Bar */}
            <div className="p-3 bg-[var(--bg-surface-elevated)] border-b border-[var(--border-subtle)] flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
              <div className="flex items-start sm:items-center gap-2.5">
                <div className="w-6 h-6 rounded-[6px] bg-[var(--brand-subtle)] border border-[var(--brand-border)] text-[var(--brand-text)] flex items-center justify-center font-semibold text-xs shrink-0 mt-0.5 sm:mt-0">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded-[6px] bg-[var(--brand-subtle)] text-[var(--brand-highlight)]">
                      Live Demo State
                    </span>
                  </div>
            ...
            <div className="p-2.5 sm:p-3 bg-[var(--bg-surface-elevated)] border-t border-[var(--border-subtle)] grid grid-cols-1 md:grid-cols-2 gap-2 text-xs">
              <div className="p-2 rounded-[10px] bg-[var(--bg-surface)] border border-[var(--border-subtle)] flex items-start gap-2">
